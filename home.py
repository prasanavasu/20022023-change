...
def login():
    ...
    if mobile and password:
        tokens = create_jwt_tokens(str(mobile))
        return ok_request(
            {
        ...
        log_console.warning("Invalid Keys")
    return notok_request({"message": "Invalid Keys"})

def create_parent(pk):
    # if not db.engine.has_table('parents'):
    #     db.create_all()
...
def create_child(pk):
    # if not db.engine.has_table('childs'):
    #     db.create_all()
...
def many_related():
    ...
    if parent and child1 and child2:
        var = get_or_create(db.session, ManyRelated, defaults=None, parent_id=int(parent), child_id1=int(child1),
                            child_id2=int(child2))
        if var:
            db.session.commit()
            return ok_request({"message": "created successfully"})
    return notok_request({"message": "Invalid Keys"})
